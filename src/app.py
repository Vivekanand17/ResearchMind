...
# Runtime sanity check to avoid importing a different/older `agents` module.
if not hasattr(agents_mod, "safe_invoke_raw"):
    st.error(
        "Backend mismatch: `agents.safe_invoke_raw` not found. "
        "Restart Streamlit after ensuring it uses the local `agents.py`."
    )
